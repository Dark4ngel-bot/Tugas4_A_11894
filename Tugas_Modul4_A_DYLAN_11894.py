...
st.set_page_config(
    page_title="11894 - Unsupervised Learning",
    page_icon="📊",
    layout="wide",
...
if uploaded_file is not None:
    ...
    st.markdown("<h1 style='text-align: center;'>Unsupervised Learning - DYLAN</h1>", unsafe_allow_html=True)
    st.dataframe(input_data)

model_directory = r"D:\File Dylan 3\ML\Unsupervised Learning (Praktek)\Tugas4_A_11894"
model_path = {
...
if st.sidebar.button("Prediksi !"):
    ...
    for i, (model_name, model, title, color_scale) in enumerate(cluster_method):
        ...
        with cols[i]:
            st.plotly_chart(fig)
            st.markdown(f"<p style='text-align: center;'>Titik Dari Data yang barumasuk ke dalam cluster : {new_cluster}</p>", unsafe_allow_html=True)